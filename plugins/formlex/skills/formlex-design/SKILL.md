...
已有术语的概念定义参考具体资料页，前端应用建议由 FormLex 整理，不将应用建议归因给资料机构。原创灵感保持原说明；待核实与用户自定义条目可经个人词库或管理范围使用，不能称为已查证术语。来源链接用于核查，不必重复写进设计简报。特色始终来自独立全局词池，不受常规分类限制。
...
结构重构：重新设计信息分组、主辅关系、导航呈现、阅读路径和页面布局，需要可辨识的结构变化。
重新设计：以现有页面提供的功能和内容为依据，从任务与设计词条重新构建界面；不默认沿用原有页面骨架或组件组合。
