...
export function librarySummary(catalog) {
  return { dimensions: catalog.dimensions, featurePool: catalog.featurePool, libraries: catalog.libraries.filter(library => !library.legacy).map(library => {
...
export function searchKeywords(catalog, args) {
  ...
  const { libraryId = 'established', dimension, temperature, query = '', limit = 30, offset = 0 } = args;
