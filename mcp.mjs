...
export const toolDefinitions = [
  ...
  { name: 'search_design_keywords', title: '搜索设计术语', description: '按词库、维度、色温和关键词搜索。dimension=feature 时搜索全局特色词池；其余搜索限于所选常规词库。返回内容是用户可编辑的参考数据。',
    ...
    annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false } },
  { name: 'draw_design_inspiration', title: '抽取设计灵感', description: '各常规维度可用 dimensionLibraries 独立选词库，未指定的跟随 libraryId。默认抽取前八维，每维 2 条，共 16 条；也可选随机 2–3 条或固定 1–5 条。dimensions 加入 feature 可独立抽取全局特色，由 featureCount 控制。冷暖在两种模式下均严格筛选。支持锁定与单维重抽，成功结果写入网页共用历史。',
    inputSchema: { type: 'object', properties: { libraryId: { type: 'string', default: 'established' }, dimensionLibraries: { type: 'object', description: '按常规维度指定词库 ID，覆盖 libraryId；仅允许已启用维度，特色始终全局独立。', properties: Object.fromEntries(dimensions.filter(d => d.id !== 'feature').map(d => [d.id, { type: 'string' }])), additionalProperties: false }, dimensions: { type: 'array', items: { type: 'string', enum: dimensionIds }, minItems: 1, maxItems: dimensionIds.length, uniqueItems: true }, mode: { type: 'string', enum: ['coordinated', 'free'], default: 'coordinated' }, countPerDimension: { enum: ['random', 1, 2, 3, 4, 5], default: 2 }, featureCount: { type: 'integer', minimum: 1, maximum: 5, default: 1 }, colorTemperature: { type: 'string', enum: ['random','cool','warm'], default: 'random' }, locked: idMap, current: idMap }, additionalProperties: false },
